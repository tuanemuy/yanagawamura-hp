...
import {
  Shop,
  useGetCategorizedShops,
  getGetCategorizedShopsPrefetcher,
  useGetCategories,
...
export async function getStaticProps({ params }: any) {
  ...
  const getCategorizedShopsPrefetcher = getGetCategorizedShopsPrefetcher({
    categoryId: id,
    limit,
    offset: 0,
  ...
  prefetches.push(
    queryClient.prefetchQuery(
      getCategorizedShopsPrefetcher.key,
      getCategorizedShopsPrefetcher.fetcher
    )
  );
...
const CategorizedShopsPage: NextPage<Props> = ({ id, categoryName, limit }) => {
  ...
  const getInitialCategorizedShops = useGetCategorizedShops({
    categoryId: id,
    limit,
    offset: 0,
    orderBy: { created_at: Order_By.Desc },
  ...
  const getCategorizedShops = useGetCategorizedShops({
    categoryId: id,
    limit,
  ...
  const totalPages =
    pageNumber === 1
      ? Math.ceil((getInitialCategorizedShops.count || 0) / limit)
      : Math.ceil((getCategorizedShops.count || 0) / limit);
  ...
  useEffect(() => {
    store.busy.setIsBusy(
      !getCategorizedShops.categorizedShops && getCategorizedShops.isFetching
    );
  }, [getCategorizedShops.categorizedShops, getCategorizedShops.isFetching]);
  ...
  return (
    <Page
      ...
      description={`「${
        categoryName || ""
      }」カテゴリーのヤナガワ村の物販・サービス店情報です。群馬県高崎市柳川町や中央銀座通り周辺の商店街・飲み屋街エリア「ヤナガワ村」には、チェーン店はほとんどありません。その代わりに様々な個性溢れる店舗が立ち並びます。昭和の香りが漂うこの街をぜひお楽しみください。`}
    ...
    >
      <Main color={colors.background} avoidHeader>
        <PageTitle title={`${categoryName} の物販・サービス店`} />

        <Stacked paddingPos="top" paddingSize="narrow" wrap isSection>
          <Panel>
            <Columns repeatXL={3} repeatM={2} repeatS={1} gap="wide">
              {(pageNumber === 1
                ? getInitialCategorizedShops.categorizedShops
                : getCategorizedShops.categorizedShops
              ).map((s: Shop, index: number) => {
                return (
                  <ShopLink
                    key={s.id}
                    shop={s}
            ...
            <Stacked paddingPos="top" paddingSize="thin">
              <SimplePagination
                ...
                setPage={(page: number) => {
                  router.push(`/shop/category/${id}?page=${page}`);
                }}
              />
            </Stacked>
          </Panel>
        ...
        <Stacked paddingPos="top" paddingSize="thin" wrap isSection>
          <Panel>
            ...
            <Stacked paddingPos="top" paddingSize="thin">
              <ToCategories postType="shop" categories={categories} selected={id} />
            </Stacked>
...
export default CategorizedShopsPage;
