...
import {
  Restaurant,
  useGetCategorizedRestaurants,
  getGetCategorizedRestaurantsPrefetcher,
  useGetCategories,
...
export async function getStaticProps({ params }: any) {
  ...
  const getCategorizedRestaurantsPrefetcher =
    getGetCategorizedRestaurantsPrefetcher({
      categoryId: id,
      limit,
      offset: 0,
  ...
  prefetches.push(
    queryClient.prefetchQuery(
      getCategorizedRestaurantsPrefetcher.key,
      getCategorizedRestaurantsPrefetcher.fetcher
    )
  );
...
const CategorizedRestaurantsPage: NextPage<Props> = ({
  id,
  categoryName,
  limit,
}) => {
  ...
  const getInitialCategorizedRestaurants = useGetCategorizedRestaurants({
    categoryId: id,
    limit,
    offset: 0,
    orderBy: { created_at: Order_By.Desc },
  ...
  const getCategorizedRestaurants = useGetCategorizedRestaurants({
    categoryId: id,
    limit,
  ...
  const totalPages =
    pageNumber === 1
      ? Math.ceil((getInitialCategorizedRestaurants.count || 0) / limit)
      : Math.ceil((getCategorizedRestaurants.count || 0) / limit);
  ...
  useEffect(() => {
    store.busy.setIsBusy(
      !getCategorizedRestaurants.categorizedRestaurants &&
        getCategorizedRestaurants.isFetching
    );
  }, [
    getCategorizedRestaurants.categorizedRestaurants,
    getCategorizedRestaurants.isFetching,
  ...
  return (
    <Page
      ...
      description={`「${
        categoryName || ""
      }」カテゴリーのヤナガワ村の飲食店情報です。群馬県高崎市柳川町や中央銀座通り周辺の商店街・飲み屋街エリア「ヤナガワ村」には、「人」を目当てに飲みに行けるような、温かい飲食店がたくさんあります。昔ながらのディープで笑顔あふれる雰囲気をぜひお楽しみください。`}
        ...
        <Stacked paddingPos="top" paddingSize="narrow" wrap isSection>
          <Panel>
            <Columns repeatXL={3} repeatM={2} repeatS={1} gap="wide">
              {(pageNumber === 1
                ? getInitialCategorizedRestaurants.categorizedRestaurants
                : getCategorizedRestaurants.categorizedRestaurants
            ...
            <Stacked paddingPos="top" paddingSize="thin">
              <SimplePagination
                ...
                setPage={(page: number) => {
                  router.push(`/restaurant/category/${id}?page=${page}`);
                }}
              />
            </Stacked>
          </Panel>
...
export default CategorizedRestaurantsPage;
